...
def _chunked_all_sum(arr: mx.array, *, max_elems: int) -> mx.array:
    ...
    n = int(arr.size)
    if n <= max_elems:
        return mx.distributed.all_sum(arr)
    flat = arr.reshape(-1)
    outs = []
    for i in range(0, n, max_elems):
        outs.append(mx.distributed.all_sum(flat[i:i + max_elems]))
    return mx.concatenate(outs).reshape(arr.shape)
