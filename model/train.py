...
def main():
    ...
    if ds_path.is_dir() and any(ds_path.glob("*.arrow")):
        ...
        if not arrow_dir.exists() and rank == 0:
            raw = load_dataset("json", data_files=str(ds_path), split="train")
            tmp = raw.map(encode_sp, fn_kwargs={"sp": sp, "key": "text"},
                          remove_columns=raw.column_names, num_proc=4)
    ...
    # model & optimizer
    model = OpenELM(cfg)
    opt   = optim.AdamW(cfg.max_lr, betas=(0.9, .95), eps=1e-8,
                        weight_decay=cfg.weight_decay)
    if cfg.torch_dtype == "bfloat16" and hasattr(mx, "set_default_dtype"):
        mx.set_default_dtype(mx.bfloat16)
    ...
    _barrier(); _broadcast_params(model.parameters(), rank)
    ...
    def loss_fn(m, batch):
        ...
        ce = losses.cross_entropy(logits.reshape(-1, cfg.vocab_size),
    ...
    for step in range(start_step + 1, cfg.max_iterations + 1):
        ...
        if step < start_step + RESTART_WARM:
            ...
            opt.learning_rate = cosine_lr(step, base=cfg.max_lr,
                                          warmup=cfg.warmup_iterations,
                                          total=cfg.max_iterations,
                                          min_lr=cfg.min_lr)
        ...
        if SCALE != 1.0:  # mini’s grads scaled
        ...
        # tiny val probe every 5k steps (rank 0 only)
