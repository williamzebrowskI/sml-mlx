...
def main():
    ...
    # ─── heterogeneous per-rank batch sizes ────────────────────
    # Ranks 0-2 (M4 Max) → 16, Rank 3 (M2 Pro) → 8
    if rank == 3:
        LOCAL_BS = 8
    else:
        LOCAL_BS = cfg.train_batch_size  # 16
    SCALE = LOCAL_BS / cfg.train_batch_size  # 0.5 for rank 3, 1.0 otherwise
    ...
    model = OpenELM(cfg)
    opt = optim.AdamW(
        cfg.max_lr, betas=(0.9, 0.95), eps=1e-8, weight_decay=cfg.weight_decay
    )
    if cfg.torch_dtype == "bfloat16" and hasattr(mx, "set_default_dtype"):
        mx.set_default_dtype(mx.bfloat16)
    ...
    # mini warm-up length after a resume
    RESTART_WARM = 1_000
    ...
    acc_l = acc_s = 0
    for step in range(start_step + 1, cfg.max_iterations + 1):
        # mini warm-up if we just resumed
        ...
        # scale grads for the mini-worker
        if SCALE != 1.0:
            grads = tree_map(
        ...
        # pretty print every 10 steps
        if step % 10 == 0 and rank == 0:
            # gather local batch sizes to confirm layout
            bs_vec = mx.distributed.all_gather(mx.array([LOCAL_BS]))
            bs_str = "/".join(str(int(x)) for x in bs_vec.tolist())
            print(
        ...
        # tiny validation probe
        if val_it and step % 5_000 == 0 and rank == 0:
            vloss = sum(float(loss_fn(model, next(val_it))) for _ in range(100)) / 100
            print(f"[val] step {step}: {vloss:.3f}", flush=True)

        # checkpoint
        if step % 2_500 == 0 and rank == 0:
            ck = out_dir / f"ckpt_{step:07d}.safetensors"
            model.save_weights(str(ck))
