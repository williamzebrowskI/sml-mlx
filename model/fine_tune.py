#!/usr/bin/env python
"""
Fine-tune TinyGPLM on GooAQ Q/A pairs using MLX.
...
def gooaq_text_iterator(split: str = "train"):
    ...
    NOTE: This assumes GooAQ has 'question' and 'answer' fields.
    If the column names differ, adjust q = ex["question"], a = ex["answer"] below.
    """
    ds = load_dataset("allenai/gooaq", split=split, streaming=True)
    ...
    for ex in ds:
        ...
        if not q or not a:
            continue
        # Simple prompt format: Q/A style
        text = f"Q: {q}\nA: {a}"
        yield {"text": text}
...
def main():
    p = argparse.ArgumentParser("Fine-tune TinyGPLM on GooAQ with MLX")
    ...
    p.add_argument("--save-dir", type=str, default="model/checkpoints_gooaq_sft", help="Where to store SFT checkpoints")
